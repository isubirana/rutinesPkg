...
#'@param sex 1:hombres, 2:mujer (o cualquier n?mero diferente de uno)
...
#'@export
calculadora.risc<-function(sex,age,coltot,hdl,tas,tad,diab,smoke,calibrated=TRUE,age.range=c(35,74)){
  ...
  attr(bp_cat,"value.labels")<-c("optima"=1,"bp_normal"=2,"alta"=3,"hipert. grado I"=4,"hipert. grado II"=5)
  bp_dum<-dummy(bp_cat)
  ...
  attr(result,"xbeta")<- ifelse(sex==1,a_chol.men,a_chol.wom)
  attr(result,"X") <- X
