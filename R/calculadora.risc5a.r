#'calculadora.risc5a
#'calculadora risc REGICOR a 5 anys
...
#'@export
calculadora.risc5a<-function(sex,age,coltot,hdl,tas,tad,diab,smoke,calibrated=TRUE,age.range=c(35,74)){
  ...
  # supervivencia basal (en la media de las covariables)
    # a 10 anys
  S0.men.10y=ifelse(!calibrated,0.90015,0.951)
  S0.wom.10y=ifelse(!calibrated,0.96246,0.978)
    # a 5 anys es divideix per dos la hazard acumulada
  H0.men.10y <- -log(S0.men.10y)
  H0.wom.10y <- -log(S0.wom.10y)
  H0.men <- H0.men.10y/2
  H0.wom <- H0.wom.10y/2
  S0.men <- exp(-H0.men)
  S0.wom <- exp(-H0.wom)
