...
#' @param files fitxer SPSS .sav
#' @author Dave McFarlane, Isaac Subirana
#' @note
...
#¡ s'han eliminat variables o creat amb versions anteriors a la 12.0).
#' @return
...
spss_varlist <- function(file){
  ...
  versiontext <- substr(IGNORE, 1, 60)
  versiontext <- unlist(strsplit(versiontext," "))
  version <- grep("^[0-9]+\\.[0-9]+\\.[0-9]+$", versiontext, value=TRUE) ## XX.XX.XX
  if (length(version)==0){
    version <- grep("^[0-9]+\\.[0-9]+\\.[0-9]+\\.[0-9]+$", versiontext, value=TRUE) ## XX.XX.XX.XX
  }
  if (length(version)==0){
    version<-11  # suposo que ?s inferior a la 12 (per exemple quan est? creada per l'STATTRANSFER no posa cap versi?)
  }else{
    version <- as.double(unlist(strsplit(version,"\\."))[1])
  }

  cat("SPSS version file:",version,"\n")
  ...
  # A199 posa A-57.
  index<-grep("A-1",printfmt)
  if (length(index)>0){
    elim<-index[!as.logical((1:length(index))%%2)]
#    printfmt[elim]<-"A1000"
    printfmt<-printfmt[-elim]
    varname<-varname[-elim]
    varlabel<-varlabel[-elim]
    nmiss<-nmiss[-elim]
    missing1<-missing1[-elim]
    missing2<-missing2[-elim]
    missing3<-missing3[-elim]
    warning("Some string variables have length>256")
  }
  printfmt<-sub("A-57","A199",printfmt)
  printfmt<-sub("A-1","A255",printfmt)
  ...
  } else longname=varname # versi? anterior a 12.0

  #treu els espais en blanc de varname, longname i printfmt
  varname <- gsub(" ","",varname)
  printfmt <- gsub(" ","",printfmt)
  longname <- gsub(" ","",longname)

  #elimina els espais per darrera i davant de les etiquetes
  ...
  if (length(index)>0) attr(dict,"elim")<-elim
